# MOVEIT VERIFICATION - CLEAN VERSION
Clear-Host
...
$subnetList = az network vnet subnet list --resource-group $resourceGroup --vnet-name vnet-moveit --output json 2>$null | ConvertFrom-Json
if ($subnetList) {
    Write-Host "  PASS - Subnet configured" -ForegroundColor Green
    $passed++
} else {
    Write-Host "  FAIL - Not found" -ForegroundColor Red
    $failed++
