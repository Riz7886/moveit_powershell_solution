...
foreach ($sub in $subs) {
    ...
    foreach ($rg in $rgs) {
        ...
        # POC
        if ($rg.ResourceGroupName -match "-poc$" -or $rg.ResourceGroupName -match "-poc-") {
            $rgEnv = "POC"
            $rgEnvTag = "poc"
        }
        # Production
        elseif ($rg.ResourceGroupName -match "-prod$" -or $rg.ResourceGroupName -match "-prod-") {
            $rgEnv = "Production"
            $rgEnvTag = "prod"
        }
        # PreProd
        elseif ($rg.ResourceGroupName -match "-preprod$" -or $rg.ResourceGroupName -match "-preprod-") {
            $rgEnv = "PreProd"
            $rgEnvTag = "preprod"
        }
        # Development
        elseif ($rg.ResourceGroupName -match "-dev$" -or $rg.ResourceGroupName -match "-dev-") {
            $rgEnv = "Development"
            $rgEnvTag = "dev"
        }
        # Test
        elseif ($rg.ResourceGroupName -match "-test$" -or $rg.ResourceGroupName -match "-test-") {
            $rgEnv = "Test"
            $rgEnvTag = "test"
        }
        # UAT
        elseif ($rg.ResourceGroupName -match "-uat$" -or $rg.ResourceGroupName -match "-uat-") {
            $rgEnv = "UAT"
            $rgEnvTag = "uat"
        }
        ...
        foreach ($r in $res) {
            ...
            if ($rgEnv -eq "POC") {
                if ($r.Name -match "-prod$") {
                    $hasIssue = $true
                    $issueDesc = "POC RG contains production resource"
                    $newName = $r.Name -replace "-prod$", "-poc"
                    $changeReason = "Resource in POC environment should have -poc suffix, not -prod"
                }
                elseif ($r.Name -match "-prod-") {
                    $hasIssue = $true
                    $issueDesc = "POC RG contains production resource"
                    $newName = $r.Name -replace "-prod-", "-poc-"
                    $changeReason = "Resource in POC environment should have -poc- identifier, not -prod-"
                }
                elseif ($r.Name -match "-preprod$") {
                    $hasIssue = $true
                    $issueDesc = "POC RG contains preprod resource"
                    $newName = $r.Name -replace "-preprod$", "-poc"
                    $changeReason = "Resource in POC environment should have -poc suffix, not -preprod"
                }
            }
            ...
            elseif ($rgEnv -eq "Production") {
                if ($r.Name -match "-dev$") {
                    $hasIssue = $true
                    $issueDesc = "Production RG contains dev resource"
                    $newName = $r.Name -replace "-dev$", "-prod"
                    $changeReason = "Resource in Production environment should have -prod suffix, not -dev"
                }
                elseif ($r.Name -match "-dev-") {
                    $hasIssue = $true
                    $issueDesc = "Production RG contains dev resource"
                    $newName = $r.Name -replace "-dev-", "-prod-"
                    $changeReason = "Resource in Production environment should have -prod- identifier, not -dev-"
                }
                elseif ($r.Name -match "-test$") {
                    $hasIssue = $true
                    $issueDesc = "Production RG contains test resource"
                    $newName = $r.Name -replace "-test$", "-prod"
                    $changeReason = "Resource in Production environment should have -prod suffix, not -test"
                }
                elseif ($r.Name -match "-preprod$") {
                    $hasIssue = $true
                    $issueDesc = "Production RG contains preprod resource"
                    $newName = $r.Name -replace "-preprod$", "-prod"
                    $changeReason = "Resource in Production environment should have -prod suffix, not -preprod"
                }
            }
            ...
            elseif ($rgEnv -eq "PreProd") {
                if ($r.Name -match "-prod$") {
                    $hasIssue = $true
                    $issueDesc = "PreProd RG contains production resource"
                    $newName = $r.Name -replace "-prod$", "-preprod"
                    $changeReason = "Resource in PreProd environment should have -preprod suffix, not -prod"
                }
                elseif ($r.Name -match "-prod-") {
                    $hasIssue = $true
                    $issueDesc = "PreProd RG contains production resource"
                    $newName = $r.Name -replace "-prod-", "-preprod-"
                    $changeReason = "Resource in PreProd environment should have -preprod- identifier, not -prod-"
                }
            }
            ...
            elseif ($rgEnv -eq "Development") {
                if ($r.Name -match "-prod$") {
                    $hasIssue = $true
                    $issueDesc = "Development RG contains production resource"
                    $newName = $r.Name -replace "-prod$", "-dev"
                    $changeReason = "Resource in Development environment should have -dev suffix, not -prod"
                }
            }
