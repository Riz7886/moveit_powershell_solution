...
<#
.SYNOPSIS
    Automated Databricks + Datadog Integration via Init Scripts
.DESCRIPTION
    Creates and deploys Datadog init scripts to Databricks clusters.
    This installs Datadog agents on Databricks cluster nodes automatically.
.PARAMETER DatadogApiKey
    Your Datadog API key
.PARAMETER DatabricksWorkspaceUrl
    Your Databricks workspace URL (e.g., https://adb-123456789.azuredatabricks.net)
.PARAMETER DatabricksToken
    Databricks Personal Access Token (create in User Settings → Access Tokens)
.PARAMETER ClusterIds
    Array of Databricks cluster IDs to configure. Leave empty to configure ALL clusters.
.EXAMPLE
    .\Configure-Databricks-Datadog.ps1 -DatadogApiKey "your-dd-key" -DatabricksWorkspaceUrl "https://adb-123.azuredatabricks.net" -DatabricksToken "your-token"
#>
...
Write-Host "`nWorkspace: $DatabricksWorkspaceUrl" -ForegroundColor Yellow
Write-Host "Datadog Site: $DatadogSite" -ForegroundColor Yellow

# ============================================================================
# STEP 1: CREATE DATADOG INIT SCRIPT
# ============================================================================

Write-Host "`n[STEP 1] Creating Datadog init script..." -ForegroundColor Cyan

$initScriptContent = @"
#!/bin/bash

# Databricks Datadog Agent Init Script
# This script installs the Datadog agent on Databricks cluster nodes
...
echo "Downloading and installing Datadog agent..."
...
if systemctl is-active --quiet datadog-agent; then
    ...
    echo "WARNING: Datadog agent may not be running. Check logs."
fi
...
# ============================================================================
# STEP 2: UPLOAD INIT SCRIPT TO DBFS
# ============================================================================

Write-Host "`n[STEP 2] Uploading init script to DBFS..." -ForegroundColor Cyan
...
# ============================================================================
# STEP 3: GET CLUSTERS TO CONFIGURE
# ============================================================================

Write-Host "`n[STEP 3] Getting Databricks clusters..." -ForegroundColor Cyan
...
# ============================================================================
# STEP 4: UPDATE CLUSTERS WITH INIT SCRIPT
# ============================================================================

Write-Host "`n[STEP 4] Configuring clusters with Datadog init script..." -ForegroundColor Cyan
...
foreach ($cluster in $clustersToUpdate) {
    Write-Host "`n  Processing: $($cluster.cluster_name) [$($cluster.cluster_id)]" -ForegroundColor White
    ...
    try {
        ...
        $currentConfig = Invoke-RestMethod -Uri "$apiUrl/clusters/get" -Method Get -Headers $headers -Body $getBody
        ...
        # Add Datadog init script to existing init scripts
        $initScripts = @()
        if ($currentConfig.init_scripts) {
        ...
        $editBody = @{
            ...
            num_workers = $currentConfig.num_workers
        ...
        Invoke-RestMethod -Uri "$apiUrl/clusters/edit" -Method Post -Headers $headers -Body $editBodyJson | Out-Null
        
        Write-Host "    ✓ Successfully updated cluster configuration" -ForegroundColor Green
        $results.Updated++
        
        # Restart cluster if it's running
        if ($currentConfig.state -eq "RUNNING") {
            Write-Host "    → Cluster is running. Restart to apply changes." -ForegroundColor Yellow
            $restartPrompt = Read-Host "      Restart now? (yes/no)"
            if ($restartPrompt -eq "yes") {
                $restartBody = @{
                    cluster_id = $cluster.cluster_id
                } | ConvertTo-Json
                
                Invoke-RestMethod -Uri "$apiUrl/clusters/restart" -Method Post -Headers $headers -Body $restartBody | Out-Null
                Write-Host "    ✓ Cluster restart initiated" -ForegroundColor Green
            }
        } else {
            Write-Host "    → Cluster is $($currentConfig.state). Start cluster to install Datadog agent." -ForegroundColor Yellow
        }
...
Write-Host "5. Check Datadog → Integrations → Spark for Spark metrics" -ForegroundColor Yellow

Write-Host "`n✅ Configuration complete!" -ForegroundColor Green
Write-Host "`nInit script location: dbfs:$dbfsPath" -ForegroundColor Cyan
