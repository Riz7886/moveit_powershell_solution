...
Write-Host ""
Write-Host "DATADOG AUTOMATION - FINAL WORKING VERSION" -ForegroundColor Cyan
Write-Host ""
...
if (-not $VMs -or $VMs.Count -eq 0) {
    ...
    Write-Host "Step 8: Installing Agents..." -ForegroundColor Cyan
    Write-Host ""
...
Write-Host ""
Write-Host "Step 9: Creating Monitors..." -ForegroundColor Cyan
Write-Host ""

$monitorUrl = "https://api.$DD_SITE.datadoghq.com/api/v1/monitor"

function New-Monitor {
    param([string]$Name, [string]$Query, [string]$Msg)
...
New-Monitor -Name "CPU High" -Query "avg(last_5m):100 - avg:system.cpu.idle{*} by {host} > 85" -Msg "CPU above 85%"
New-Monitor -Name "Memory Low" -Query "avg(last_5m):avg:system.mem.usable{*} by {host} < 500000000" -Msg "Memory below 500MB"
New-Monitor -Name "Disk High" -Query "avg(last_5m):avg:system.disk.in_use{*} by {host,device} > 0.90" -Msg "Disk above 90%"
...
Write-Host "Monitors created: 3" -ForegroundColor Green
Write-Host ""
Write-Host "WAIT 10 MINUTES" -ForegroundColor Yellow
Write-Host ""
Write-Host "Then go to: https://$DD_SITE.datadoghq.com/monitors/manage" -ForegroundColor Cyan
Write-Host "Monitors will show GREEN" -ForegroundColor Green
Write-Host ""
