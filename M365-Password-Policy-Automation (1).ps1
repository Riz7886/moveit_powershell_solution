...
function Set-PasswordExpirationPolicy {
    ...
    # Check and install MSOnline module
    ...
    Write-Host "MSOnline module ready!" -ForegroundColor Green
    ...
    try {
        ...
        Write-Host ""
        Write-Host "Getting verified domains..." -ForegroundColor Yellow
        $domains = Get-MsolDomain | Where-Object { $_.Status -eq "Verified" }
        
        Write-Host ""
        Write-Host "Found $($domains.Count) verified domains:" -ForegroundColor Cyan
        foreach ($domain in $domains) {
            Write-Host "  - $($domain.Name)" -ForegroundColor White
        }
        
        # Set password policy for each domain
        Write-Host ""
        Write-Host "Setting 90-day password policy..." -ForegroundColor Yellow
        Write-Host ""
        ...
        foreach ($domain in $domains) {
            try {
                Set-MsolPasswordPolicy -DomainName $domain.Name -ValidityPeriod 90 -NotificationDays 14
                Write-Host "  [OK] $($domain.Name)" -ForegroundColor Green
                Write-Host "       - Password expires: 90 days" -ForegroundColor Gray
                Write-Host "       - Notification: 14 days before" -ForegroundColor Gray
                $successDomains++
            } catch {
                Write-Host "  [SKIP] $($domain.Name) - May be federated or managed differently" -ForegroundColor Yellow
                $failedDomains++
            }
        }
        ...
        Write-Host "==========================================================" -ForegroundColor Green
        Write-Host ""
        Write-Host "  Domains Updated: $successDomains" -ForegroundColor Green
        Write-Host "  Domains Skipped: $failedDomains" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "  Policy: Passwords expire every 90 days" -ForegroundColor Cyan
        Write-Host "  Notification: Users notified 14 days before expiry" -ForegroundColor Cyan
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "If connection failed, try running PowerShell as Administrator." -ForegroundColor Yellow
    }
}
    ...
    switch ($choice) {
        ...
        "2" {
            ...
            if ($reports -and $reports.UsersToReset.Count -gt 0) {
                # Show users to reset and allow manual exclusion
                Write-Host ""
                Write-Host "==========================================================" -ForegroundColor Yellow
                Write-Host "  REVIEW USERS BEFORE RESET" -ForegroundColor Yellow
                Write-Host "==========================================================" -ForegroundColor Yellow
                Write-Host ""
                Write-Host "Check the report: USERS-To-Reset CSV on your Desktop" -ForegroundColor Cyan
                Write-Host ""
                Write-Host "If you see ANY application/service accounts in that list," -ForegroundColor Yellow
                Write-Host "enter their usernames below to EXCLUDE them." -ForegroundColor Yellow
                Write-Host ""
                Write-Host "Enter usernames to exclude (comma separated) or press ENTER to skip:" -ForegroundColor White
                $manualExclusions = Read-Host
                
                if ($manualExclusions) {
                    $excludeList = $manualExclusions -split "," | ForEach-Object { $_.Trim() }
                    $originalCount = $reports.UsersToReset.Count
                    
                    $reports.UsersToReset = $reports.UsersToReset | Where-Object {
                        $upn = $_.UserPrincipalName
                        $name = $_.DisplayName
                        $exclude = $false
                        foreach ($ex in $excludeList) {
                            if ($upn -like "*$ex*" -or $name -like "*$ex*") {
                                Write-Host "  EXCLUDED: $name ($upn)" -ForegroundColor Magenta
                                $exclude = $true
                                break
                            }
                        }
                    ...
                    $excludedCount = $originalCount - $reports.UsersToReset.Count
                    Write-Host ""
                    Write-Host "Manually excluded: $excludedCount accounts" -ForegroundColor Magenta
                    Write-Host "Remaining to reset: $($reports.UsersToReset.Count)" -ForegroundColor Green
                }
