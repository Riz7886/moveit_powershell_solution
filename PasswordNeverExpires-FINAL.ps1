...
Write-Host "  AZURE ENTRA ID - PASSWORD POLICY DETECTION" -ForegroundColor Yellow
Write-Host "  Finding users with PASSWORD NEVER EXPIRES policy" -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
function Get-UsersWithPasswordPolicy {
    Write-Host "[3/4] Scanning all users..." -ForegroundColor Cyan
    Write-Host "  Looking for users with PASSWORD NEVER EXPIRES policy..." -ForegroundColor Yellow
    Write-Host ""
    ...
    try {
        ...
        foreach ($user in $users) {
            ...
            if ($passwordNeverExpires) {
                ...
                $userObject = [PSCustomObject]@{
                    ...
                    PasswordPolicies = if ($user.PasswordPolicies) { $user.PasswordPolicies } else { "None" }
        ...
        Write-Host ""
        Write-Host "  Found $usersWithPolicy users with PASSWORD NEVER EXPIRES policy" -ForegroundColor Green
        Write-Host ""
...
function Export-ToHTML {
    ...
    try {
        ...
        $usersOver365Days = ($Data | Where-Object { $_.PasswordAgeDays -ne "N/A" -and [int]$_.PasswordAgeDays -gt 365 }).Count
...
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card"><div class="stat-number warning">$usersOver180Days</div><div class="stat-label">Over 180 Days</div></div>
            <div class="stat-card"><div class="stat-number danger">$usersOver365Days</div><div class="stat-label">Over 1 Year</div></div>
        </div>
        <div class="alert">
            <h3>PASSWORD NEVER EXPIRES POLICY APPLIED</h3>
            <p><strong>$totalUsers users</strong> have the Password Never Expires policy applied. Their passwords will NOT expire automatically.</p>
            <p>These are typically service accounts, admin accounts, or special purpose accounts.</p>
        </div>
...
"@
        ...
        foreach ($user in $Data) {
            ...
            $htmlBody += "<td><strong style='color: #0c5460;'>YES</strong></td>"
...
$htmlFooter = @"
        ...
        <div class="footer">
            ...
            <p>These users have the policy applied and passwords will NOT expire</p>
        </div>
    </div>
</body>
</html>
...
$userData = Get-UsersWithPasswordPolicy
...
$csvPath = Join-Path $OutputFolder "PasswordNeverExpires_$timestamp.csv"
$htmlPath = Join-Path $OutputFolder "PasswordNeverExpires_$timestamp.html"
...
Write-Host "============================================================" -ForegroundColor Green
Write-Host ""
Write-Host "  Total Users with Policy: $($userData.Count)" -ForegroundColor White
Write-Host ""
