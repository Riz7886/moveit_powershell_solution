...
function Install-RequiredModules {
    ...
    $requiredModules = @(
        @{Name = "Microsoft.Graph"; SubModules = @("Microsoft.Graph.Users", "Microsoft.Graph.Groups", "Microsoft.Graph.Identity.SignIns")},
        @{Name = "Az.Accounts"; SubModules = @()},
        @{Name = "Az.Resources"; SubModules = @()}
    )
    
    foreach ($moduleInfo in $requiredModules) {
        $moduleName = $moduleInfo.Name
        
        Write-Log "Checking for $moduleName..." "INFO"
        
        if (-not (Get-Module -ListAvailable -Name $moduleName)) {
            Write-Log "Module $moduleName not found. Installing..." "WARNING"
            try {
                ...
                Write-Log "Please install manually: Install-Module -Name $moduleName -Force -AllowClobber -Scope CurrentUser" "ERROR"
                return $false
            }
        }
        else {
            Write-Log "$moduleName is already installed" "SUCCESS"
            ...
            try {
                ...
                Write-Log "Failed to import $moduleName : $_" "ERROR"
                return $false
            }
        }
        
        # Install/check submodules if specified
        foreach ($subModule in $moduleInfo.SubModules) {
            if (-not (Get-Module -ListAvailable -Name $subModule)) {
                Write-Log "Installing sub-module $subModule..." "INFO"
                try {
                    Install-Module -Name $subModule -Force -AllowClobber -Scope CurrentUser -ErrorAction Stop
                    Import-Module -Name $subModule -Force -ErrorAction Stop
                    Write-Log "Successfully installed $subModule" "SUCCESS"
                }
                catch {
                    Write-Log "Failed to install $subModule : $_" "WARNING"
                }
            }
        }
    }
...
function Start-Audit {
    ...
    if (-not (Install-RequiredModules)) {
        Write-Log "Failed to install required modules. Exiting." "ERROR"
        Write-Host ""
        Write-Host "Please run the following commands manually:" -ForegroundColor Yellow
        Write-Host "Install-Module -Name Microsoft.Graph -Force -AllowClobber -Scope CurrentUser" -ForegroundColor Yellow
        Write-Host "Install-Module -Name Az.Accounts -Force -AllowClobber -Scope CurrentUser" -ForegroundColor Yellow
        Write-Host ""
        return
    }
