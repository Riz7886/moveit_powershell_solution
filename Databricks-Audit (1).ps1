...
# ============================================================
# STEP 7: GET COST DATA (LAST 5 MONTHS)
# ============================================================
Write-Host ""
Write-Host "Step 7: Getting cost data (last 5 months)..." -ForegroundColor Yellow
...
foreach ($rg in $rgNames) {
    Write-Host "  Costs for: $rg" -ForegroundColor Gray
    try {
        $endDate = Get-Date
        ...
        if ($usage) {
            ...
            foreach ($g in $grouped) {
                $mCost = [math]::Round(($g.Group | Measure-Object -Property PretaxCost -Sum).Sum, 2)
                $curr = "USD"
                if ($g.Group[0].Currency) { $curr = $g.Group[0].Currency }
                $costData += [PSCustomObject]@{ Month=$g.Name; RG=$rg; Cost=$mCost; Currency=$curr }
                $totalCost5Mo += $mCost
            }
        }
    } catch {
        Write-Host "    (Cost data requires Cost Management Reader role)" -ForegroundColor DarkGray
    }
}

# Calculate monthly average and projections
$avgMonthlyCost = 0
if ($costData.Count -gt 0) {
    $monthlyTotals = $costData | Group-Object Month | ForEach-Object {
        [PSCustomObject]@{ Month=$_.Name; Total=[math]::Round(($_.Group | Measure-Object -Property Cost -Sum).Sum, 2) }
    }
    $monthlyCosts = $monthlyTotals | Sort-Object Month
    $avgMonthlyCost = [math]::Round($totalCost5Mo / $monthlyCosts.Count, 2)
}

# Project future costs (with 10% annual increase assumption)
...
# Potential savings from idle resources
$potentialSavings = 0
foreach ($i in $idle) {
    if ($i.Type -eq "VM") { $potentialSavings += 150 }
    if ($i.Type -eq "Disk") { $potentialSavings += 20 }
    if ($i.Type -eq "PublicIP") { $potentialSavings += 5 }
    if ($i.Type -eq "NIC") { $potentialSavings += 0 }
}
$potentialSavings = $potentialSavings * 5

Write-Host "  Total (5 months): `$$totalCost5Mo" -ForegroundColor Cyan
Write-Host "  Avg Monthly: `$$avgMonthlyCost" -ForegroundColor Cyan
...
[void]$h.Append("<div class='card gc'><h3>5 Mo Cost</h3><div class='n'>`$$totalCost5Mo</div><div class='s'>USD</div></div>")
...
[void]$h.Append("<div class='sec'><h2>Cost Analysis - Last 5 Months</h2>")
...
[void]$h.Append("<tr style='background:#f0f0f0;font-weight:bold'><td>TOTAL (5 Months)</td><td>`$$totalCost5Mo</td></tr>")
[void]$h.Append("<tr style='background:#e8f5e9;font-weight:bold'><td>Average Monthly</td><td>`$$avgMonthlyCost</td></tr>")
[void]$h.Append("</table>")
}else{
[void]$h.Append("<p>Cost data not available. Requires Cost Management Reader role.</p>")
}
[void]$h.Append("</div>")

# 3-YEAR PROJECTION SECTION
[void]$h.Append("<div class='sec'><h2>3-Year Cost Projection</h2>")
[void]$h.Append("<p style='color:#666;font-size:12px'>Based on current usage with 10% annual increase assumption</p>")
[void]$h.Append("<table><tr><th>Period</th><th>Projected Cost (USD)</th></tr>")
[void]$h.Append("<tr><td>Year 1 (Next 12 months)</td><td>`$$year1Cost</td></tr>")
[void]$h.Append("<tr><td>Year 2</td><td>`$$year2Cost</td></tr>")
[void]$h.Append("<tr><td>Year 3</td><td>`$$year3Cost</td></tr>")
[void]$h.Append("<tr style='background:#fff3cd;font-weight:bold'><td>TOTAL 3-YEAR COST</td><td>`$$total3YearCost</td></tr>")
[void]$h.Append("</table></div>")

# POTENTIAL SAVINGS SECTION
[void]$h.Append("<div class='sec'><h2>Potential Savings from Idle Resources</h2>")
[void]$h.Append("<table><tr><th>Resource Type</th><th>Count</th><th>Est. Monthly Cost</th></tr>")
$idleVMs = @($idle | Where-Object {$_.Type -eq "VM"}).Count
$idleDisks = @($idle | Where-Object {$_.Type -eq "Disk"}).Count
$idlePIPs = @($idle | Where-Object {$_.Type -eq "PublicIP"}).Count
$idleNICs = @($idle | Where-Object {$_.Type -eq "NIC"}).Count
[void]$h.Append("<tr><td>Stopped VMs</td><td>$idleVMs</td><td>`$$($idleVMs * 150)/mo</td></tr>")
[void]$h.Append("<tr><td>Unattached Disks</td><td>$idleDisks</td><td>`$$($idleDisks * 20)/mo</td></tr>")
[void]$h.Append("<tr><td>Unassociated Public IPs</td><td>$idlePIPs</td><td>`$$($idlePIPs * 5)/mo</td></tr>")
[void]$h.Append("<tr><td>Unattached NICs</td><td>$idleNICs</td><td>`$0/mo</td></tr>")
$monthlySavings = ($idleVMs * 150) + ($idleDisks * 20) + ($idlePIPs * 5)
$yearlySavings = $monthlySavings * 12
[void]$h.Append("<tr style='background:#d4edda;font-weight:bold'><td>POTENTIAL MONTHLY SAVINGS</td><td></td><td>`$$monthlySavings/mo</td></tr>")
[void]$h.Append("<tr style='background:#c3e6cb;font-weight:bold'><td>POTENTIAL YEARLY SAVINGS</td><td></td><td>`$$yearlySavings/yr</td></tr>")
[void]$h.Append("</table></div>")
...
if($costData.Count -gt 0){$costData|Export-Csv (Join-Path $ReportPath "CostData-$Timestamp.csv") -NoTypeInformation}
...
Write-Host "  5-MO COST:  `$$totalCost5Mo" -ForegroundColor Green
Write-Host "  3-YR PROJ:  `$$total3YearCost" -ForegroundColor Green
Write-Host ""
