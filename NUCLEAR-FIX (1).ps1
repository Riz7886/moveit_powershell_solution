...
foreach ($O in $Origins) {
    ...
    if ($O.hostName -eq "192.168.0.5" -or $O.hostName -eq "20.86.24.168" -or $O.hostName -eq "20.86.24.141") {
...
Write-Host "  Creating origin with correct IP: 20.86.24.164" -ForegroundColor White
az afd origin create --profile-name $FD --resource-group $RG --origin-group-name $OGName --origin-name moveit-backend --host-name 20.86.24.164 --origin-host-header 20.86.24.164 --priority 1 --weight 1000 --enabled-state Enabled --http-port 80 --https-port 443 --output none 2>$null
Write-Host "[OK] Origin IP fixed to 20.86.24.164" -ForegroundColor Green
Write-Host ""
...
Write-Host "  - Origin IP changed to 20.86.24.164 (CORRECT!)" -ForegroundColor White
