param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$SubscriptionName = "sub-product-prod",
...
function Connect-ToAzure {
    ...
    try {
        ...
        Write-Host "  Setting subscription..." -ForegroundColor White
        Write-Host "    Target Subscription ID: $SubscriptionId" -ForegroundColor Gray
        Write-Host "    Target Subscription Name: $SubscriptionName" -ForegroundColor Gray
        ...
        Write-Host "  ✓ Tenant: $($sub.Tenant.Id)" -ForegroundColor Gray
        Write-Host ""
...
function Get-ResourceGroupDetails {
    ...
    try {
        Write-Host "  Searching for resource group: $ResourceGroupName" -ForegroundColor White
        Write-Host "  In subscription: $SubscriptionId" -ForegroundColor White
        Write-Host ""
        ...
        Write-Host "  ✓ Resource ID: $($rg.ResourceId)" -ForegroundColor Gray
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "  Listing all resource groups in subscription..." -ForegroundColor Yellow
        ...
        try {
            ...
            foreach ($group in $allRGs) {
                Write-Host "    - $($group.ResourceGroupName)" -ForegroundColor White
            }
            Write-Host ""
            Write-Host "  Please check the resource group name and try again." -ForegroundColor Yellow
        } catch {
            Write-Host "  Could not list resource groups: $($_.Exception.Message)" -ForegroundColor Red
        }
