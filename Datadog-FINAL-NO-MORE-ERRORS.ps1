...
function Create-SafeMonitor {
    ...
    try {
        ...
        if ($found) {
            Write-Host "  [EXISTS] Already created" -ForegroundColor Yellow
            return $true
        }
    } catch {
        # If we can't list monitors, just try to create
    }
    ...
    try {
        ...
        Write-Host "  [EXISTS] (duplicate detected)" -ForegroundColor Yellow
        return $true
    }
}
...
Write-Host "Monitors Total: $monitorCount" -ForegroundColor Green
Write-Host ""
