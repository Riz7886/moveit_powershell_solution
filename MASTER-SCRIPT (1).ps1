...
for ($i = 1; $i -le 3; $i++) {
    ...
    $testMetrics = @{
        series = @(
            @{
                metric = "custom.databricks.test.metric"
                points = @(,@($now, $i * 10))
                type = "gauge"
...
Write-Host ""
Write-Host "Test metrics sent! In 3 minutes, check Datadog Metrics Explorer for: custom.databricks.test.metric" -ForegroundColor Cyan
Write-Host ""
...
while ($true) {
    ...
    try {
        ...
        foreach ($cluster in $clusters) {
            ...
            $allMetrics += @{
                metric = "custom.databricks.cluster.health"
            ...
            $allMetrics += @{
                metric = "custom.databricks.dbu.usage"
                points = @(,@($now, $dbuValue))
                type = "gauge"
                tags = @("cluster:$clusterName", "cluster_id:$clusterId", "service:databricks", "env:prod")
            }
            ...
            $apiErrors = 0
            $allMetrics += @{
                metric = "custom.databricks.api.errors"
            ...
            $allMetrics += @{
                metric = "custom.databricks.api.latency"
            ...
            # Jobs
            $allMetrics += @{
                metric = "custom.databricks.jobs.failing"
                points = @(,@($now, 0))
                type = "count"
                tags = @("cluster:$clusterName", "service:databricks", "env:prod")
            }
            ...
            $allMetrics += @{
                metric = "custom.databricks.jobs.queued"
