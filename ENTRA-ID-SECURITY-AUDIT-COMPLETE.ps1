...
# ============================================================================
# Purpose: Complete Entra ID (Azure AD) security audit including:
#          - User accounts, licenses, and security
#          - Groups, roles, and memberships
#          - Conditional Access policies
#          - MFA enrollment and compliance
#          - Enterprise Applications (SSO/SAML/OIDC)
...
function Test-ModuleInstalled {
    ...
    if ($module) {
        Write-AuditLog "Module '$ModuleName' is installed (Version: $($module.Version))" -Type "SUCCESS"
        return $true
    } else {
...
function Install-RequiredModules {
    ...
    foreach ($module in $requiredModules) {
        ...
        if (-not $installed) {
            Write-AuditLog "Installing module: $($module.Name)..." -Type "WARNING"
            try {
                Install-Module -Name $module.Name -Force -AllowClobber -Scope CurrentUser -ErrorAction Stop
                Write-AuditLog "Successfully installed $($module.Name)" -Type "SUCCESS"
            }
            catch {
                Write-AuditLog "Failed to install $($module.Name): $_" -Type "ERROR"
            }
        } else {
            Write-AuditLog "Module $($module.Name) already installed" -Type "SUCCESS"
        }
    }
}
...
function Get-UserAudit {
    ...
    try {
        # Get all users
        $users = Get-MgUser -All -Property DisplayName,UserPrincipalName,Mail,AccountEnabled,CreatedDateTime,
        ...
        # Inactive users (no sign-in in last 90 days)
        ...
        if ($inactiveUsers.Count -gt 0) {
            Add-Finding -Severity "Medium" -Category "User Security" `
                ...
                -AffectedItems ($inactiveUsers | Select-Object -First 10 UserPrincipalName).UserPrincipalName
        ...
        if ($noPasswordExpiry.Count -gt 0) {
            Add-Finding -Severity "High" -Category "Password Policy" `
                ...
                -Recommendation "Enable password expiration for all users unless there's a specific business reason" `
                -Impact "Non-expiring passwords increase security risk" `
                -AffectedItems ($noPasswordExpiry | Select-Object -First 10 UserPrincipalName).UserPrincipalName
...
function Get-GroupAudit {
    ...
    try {
        $groups = Get-MgGroup -All -Property DisplayName,Description,GroupTypes,Mail,
                                             SecurityEnabled,MembershipRule,OnPremisesSyncEnabled
    ...
    try {
        ...
        if ($globalAdmins -and $globalAdmins.MemberCount -gt 5) {
            Add-Finding -Severity "High" -Category "Privileged Access" `
                ...
                -AffectedItems ($globalAdmins.Members.UPN | Select-Object -First 10)
    ...
    try {
        ...
        foreach ($user in $users | Select-Object -First 200) {
            try {
                ...
                Write-AuditLog "Could not get MFA status for $($user.UserPrincipalName)" -Type "WARNING"
            }
        }
...
function Get-EnterpriseAppsAudit {
    ...
    try {
        $apps = Get-MgServicePrincipal -All -Property AppId,DisplayName,ServicePrincipalType,
                                                      SignInAudience,AccountEnabled,Tags
...
function Get-LicenseAudit {
    ...
    try {
        ...
        if ($availableLicenses -gt 0) {
            $estimatedWaste = $availableLicenses * 10 # Assuming avg $10/license/month
...
function Get-CostAnalysis {
    if ($SkipCostAnalysis) {
        Write-AuditLog "Cost analysis skipped per parameter" -Type "INFO"
        return
    }
    
    Write-AuditLog "Performing cost analysis..." -Type "INFO"
    ...
    try {
        ...
        foreach ($sub in $subscriptions | Select-Object -First 3) {
            try {
                ...
                Write-AuditLog "Cost analysis for subscription: $($sub.Name)" -Type "INFO"
                
            }
            catch {
                Write-AuditLog "Could not get cost data for $($sub.Name): $_" -Type "WARNING"
            }
        }
        
        $script:AuditResults.CostAnalysis = @{
            AnalysisDate = Get-Date
            TotalMonthlyEstimate = $totalCost
            RecommendedSavings = @()
        }
        
        Write-AuditLog "Cost analysis completed" -Type "SUCCESS"
    }
    catch {
        Write-AuditLog "Failed to perform cost analysis: $_" -Type "ERROR"
    }
}
...
function Generate-Recommendations {
    ...
    # Privileged access recommendation
    ...
    <div class="container">
        ...
        <div class="score-section">
            ...
            <div style="margin-top: 20px; font-size: 1.1em;">
                $(
                    switch ($script:AuditResults.SecurityScore.Overall) {
        ...
        <div class="content">
            ...
            <div class="tenant-info">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <div>$($script:AuditResults.TenantInfo.DisplayName)</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Tenant ID</div>
                        <div>$($script:AuditResults.TenantInfo.TenantId)</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Primary Domain</div>
                        <div>$($script:AuditResults.TenantInfo.VerifiedDomains)</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Directory Sync</div>
                        <div>$(if ($script:AuditResults.TenantInfo.DirectorySyncEnabled) {"✅ Enabled"} else {"❌ Disabled (Cloud-Only)"})</div>
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        ...
                        <div class="stat-value">$($script:AuditResults.Statistics.Users.Total)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Active Users</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Users.Enabled)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Inactive (90 days)</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Users.Inactive90Days)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Guest Users</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Users.Guest)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Licensed Users</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Users.Licensed)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Cloud-Only Users</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Users.CloudOnly)</div>
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        <div class="stat-label">MFA Enrollment Rate</div>
                        <div class="stat-value">$($script:AuditResults.MFA.MFAPercentage)%</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">MFA Enabled</div>
                        <div class="stat-value">$($script:AuditResults.MFA.MFAEnabled)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">MFA Not Enabled</div>
                        <div class="stat-value">$($script:AuditResults.MFA.MFADisabled)</div>
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        ...
                        <div class="stat-value">$($script:AuditResults.Statistics.Groups.Total)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Security Groups</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Groups.Security)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Microsoft 365 Groups</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Groups.Microsoft365)</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Dynamic Groups</div>
                        <div class="stat-value">$($script:AuditResults.Statistics.Groups.Dynamic)</div>
                    </div>
                </div>
            </div>
...
"@
                        ...
                        '<p style="color: red; font-weight: bold;">❌ No Conditional Access policies configured</p>'
                    }
                )
            </div>
...
"@
                        foreach ($finding in $allFindings) {
                            @"
                                <tr>
                                    <td><span class="severity-badge severity-$($finding.Severity.ToLower())">$($finding.Severity)</span></td>
...
"@
                        ...
                        '<p style="color: green; font-weight: bold;">✅ No critical security findings</p>'
                    }
                )
            </div>
...
"@
        ...
        <div class="footer">
            ...
            <p style="margin-top: 15px; font-size: 0.9em;">
                ...
                Review findings with your security team and implement recommendations promptly.
            </p>
        </div>
    </div>
</body>
